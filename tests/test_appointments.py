...
def test_overlapping_appointment_rejected(client):
    ...
    assert client.post("/appointments", json=first).status_code == 201
    assert client.post("/appointments", json=second).status_code == 409
...
def test_back_to_back_appointment_allowed(client):
    ...
    assert client.post("/appointments", json=first).status_code == 201
    assert client.post("/appointments", json=second).status_code == 201
...
def test_missing_patient_for_appointment(client):
    doctor_id = create_doctor(client)
...
def test_missing_doctor_for_appointment(client):
    patient_id = create_patient(client)
